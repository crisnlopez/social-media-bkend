...
func (d DB) CreatePost(userEmail, text string) (Post, error) {
  // Check if user exist 
  var emptyUser User
  user, err := d.GetUser(userEmail)
  if err != nil {
    return Post{}, err
  }
  if user == emptyUser {
  ...
  result, err := d.Db.Exec("insert into posts (id, userEmail, postText) values (?, ?, ?)", post.ID, post.UserEmail, post.Text)
  if err != nil {
    return Post{}, err
  }
  result.RowsAffected()
...
func (d DB) GetPost(userEmail string) ([]Post, error){
  var posts []Post

  rows, err := d.Db.Query("select * from posts where userEmail = ?", userEmail)
  if err != nil {
